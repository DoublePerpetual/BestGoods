      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-1">
          ...
          ${currentPage < totalPages ? `
            <a href="/?region=${region}&search=${encodeURIComponent            <a href="/?region=${region}&search=${encodeURIComponent(search)}&level1=${level1}&level2=${level2}&page=${currentPage+1}" 
               class="px-3 py-2 border border-gray-300 rounded-lg text-sm hover:bg-gray-50">
              下一页
            </a>
          ` : ''}
...
app.listen(PORT, () => {
  ...
  console.log(`🎯 搜索功能:`);
  console.log(`   - 全局搜索: 无论用户在哪个页面搜索，都视为对24.5万品类进行全局搜索`);
  console.log(`   - 每页20行: 搜索结果每页显示20行`);
  console.log(`   - 智能分页: 搜索结果分页显示，每页20个`);
  console.log(`   - 搜索提示: 显示搜索统计信息和每页显示行数`);
});
