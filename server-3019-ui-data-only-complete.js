...
app.get('/', (req, res) => {
  ...
  if (view === 'grid') {
    ...
    res.send(renderListPage(page, region, search));
  }
});
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
      <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
        ...
        <div class="flex gap-2">
          ...
          <!-- 地区切换 - 保持3019设计 -->
          <div class="flex items-center bg-gray-100 p-1 rounded-lg">
            <a href="/?view=grid&region=all&search=${search}&level1=${level1}&level2=${level2}" class="px-3 py-1.5 rounded-md text-sm ${region === 'all' ? 'bg-white shadow' : 'text-gray-600'}">全部</a>
            <a href="/?view=grid&region=global&search=${search}&level1=${level1}&level2=${level2}" class="px-3 py-1.5 rounded-md text-sm ${region === 'global' ? 'bg-white shadow' : 'text-gray-600'}">全球</a>
            <a href="/?view=grid&region=china&search=${search}&level1=${level1}&level2=${level2}" class="px-3 py-1.5 rounded-md text-sm ${region === 'china' ? 'bg-white shadow' : 'text-gray-600'}">中国</a>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-wrap gap-2 mt-4">
        <a href="/?view=grid&region=${region}&search=${search}&level1=all&level2=all" 
