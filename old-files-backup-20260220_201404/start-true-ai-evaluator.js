...
const CONFIG = {
  ...
  outputFile: path.join(__dirname, 'data/best-answers.json'),  // 使用标准文件名
  logFile: path.join(__dirname, 'logs/true-ai-processing.log'),
  statusFile: path.join(__dirname, 'data/automation-status.json')
};
    ...
    try {
      ...
      if (fs.existsSync(CONFIG.outputFile)) {
        try {
          ...
          this.log(`⚠️ 读取现有数据失败，创建新文件: ${error.message}`);
        }
      }
      
      // 合并结果（基于品类路径去重）
      const mergedResults = [...existingResults];
      const existingPaths = new Set(existingResults.map(item => 
        `${item.level1} > ${item.level2} > ${item.item}`
      ));
      
      for (const newResult of this.results) {
        const newPath = `${newResult.level1} > ${newResult.level2} > ${newResult.item}`;
      ...
      // 保存合并后的结果
      fs.writeFileSync(CONFIG.outputFile, JSON.stringify(mergedResults, null, 2));
      
      // 计算质量统计（基于所有验证，不仅仅是当前的）
      const qualityStats = this.qualityValidator.getStats();
      
      // 更新自动化状态（包含质量统计）
      const status = {
        totalCategories: this.categories.length,
        completedCategories: mergedResults.length, // 使用合并后的总数
        bestProductsCount: mergedResults.length,
      ...
      this.log(`💾 进度已保存: ${mergedResults.length}/${this.categories.length} (${((mergedResults.length / this.categories.length) * 100).toFixed(2)}%)`);
