...
app.get('/preview', (req, res) => {
  ...
  // 生成简洁对比表格
  let comparisonTableHTML = '<table class="w-full text-sm border border-gray-200"><thead class="bg-gray-50"><tr><th class="px-4 py-3 text-left font-bold text-gray-700 border-b border-gray-200">价格区间</th>';
  
  evaluationDimensions.forEach(dim => {
    const color = colorMap[dim.color];
    comparisonTableHTML += `<th class="px-4 py-3 text-center font-bold ${color.text} border-b border-gray-200">${dim.name}</th>`;
  });
  
  comparisonTableHTML += '</tr></thead><tbody>';
  
  priceIntervals.forEach(price => {
    comparisonTableHTML += `<tr class="hover:bg-gray-50"><td class="px-4 py-3 font-bold text-gray-900 border-r border-b border-gray-200">${price.name} (${price.range})</td>`;
    
    evaluationDimensions.forEach(dim => {
      const product = bestProducts.find(p => p.priceId === price.id && p.dimensionId === dim.id);
      comparisonTableHTML += `<td class="px-4 py-3 text-center border-b border-gray-200">${product ? `${product.name}<br><span class="text-xs text-gray-500">${product.price}</span>` : '-'}</td>`;
    });
...
app.listen(PORT, () => {
  ...
  console.log('   5. 白色背景 - 所有模块使用白色背景，统一简洁');
  console.log('   6. 彩色文字 - 用彩色文字和边框进行区分');
});    
    <!-- 评论区域 -->
    <div class="p-5 bg-white rounded-lg border border-gray-200">
      ...
      <div class="space-y-4">
        ...
        <div class="p-4 border border-gray-200 rounded-lg">
          ...
          <div class="text-gray-700">页面设计简洁美观，信息一目了然，用户体验很好。</div>
        </div>
      </div>
